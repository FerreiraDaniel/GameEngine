...
/**
 * Helps to create the matrix to pass in the render as GL V1 has but Adapted for
 * GL V2
 */
public class GLTransformation {
	...
	public GLTransformation() {
		mMatrixFloatBuffer = ByteBuffer.allocateDirect(MATRIX_SIZE * FLOAT_SIZE).order(ByteOrder.nativeOrder())
				.asFloatBuffer();
	}
	...
	public void translate(float x, float y, float z) {
		...
		mMatrix[12] += (mMatrix[0] * x + mMatrix[4] * y + mMatrix[2 * 4] * z);
		mMatrix[13] += (mMatrix[1] * x + mMatrix[5] * y + mMatrix[2 * 4 + 1] * z);
		mMatrix[14] += (mMatrix[2] * x + mMatrix[6] * y + mMatrix[2 * 4 + 2] * z);
		mMatrix[15] += (mMatrix[3] * x + mMatrix[7] * y + mMatrix[2 * 4 + 3] * z);
	}
	 ...
	 * @param x
	 *            Specify the factor in x coordinate
	 * @param y
	 *            Specify the factor in y coordinate
	 * @param z
	 *            Specify the factor in z coordinate
	 */
	 ...
	 *
	 * @param left
	 *            Specify the coordinate for the vertical clipping plane.
	 * @param right
	 *            Specify the coordinate for the right vertical clipping plane.
	 * @param bottom
	 *            Specify the coordinate for the bottom horizontal clipping
	 *            plane.
	 * @param top
	 *            Specify the coordinate for the horizontal clipping plane.
	 * @param nearZ
	 *            Specify the distance to the near clipping plane. Distances
	 *            must be positive.
	 * @param farZ
	 *            Specify the distance to the far depth clipping plane.
	 *            Distances must be positive.
	 */
	private void frustum(float left, float right, float bottom, float top, float nearZ, float farZ) {
	...
	/**
	 * Multiply the current matrix by a matrix that should transform the
	 ...
	 *            direction.
	 * @param aspect
	 *            specifies the aspect ration that determines the field of view
	...
	private void multiplyMatrix(float[] mtlA, float[] mtlB) {
		...
		for (i = 0; i < MATRIX_SIDE; i++) {
			tmp[i * MATRIX_SIDE] = (mtlA[i * 4] * mtlB[0]) + (mtlA[i * 4 + 1] * mtlB[4])
					+ (mtlA[i * 4 + 2] * mtlB[2 * 4]) + (mtlA[i * 4 + 3] * mtlB[3 * 4]);

			tmp[i * MATRIX_SIDE + 1] = (mtlA[i * 4] * mtlB[1]) + (mtlA[i * 4 + 1] * mtlB[5])
					+ (mtlA[i * 4 + 2] * mtlB[2 * 4 + 1]) + (mtlA[i * 4 + 3] * mtlB[3 * 4 + 1]);

			tmp[i * MATRIX_SIDE + 2] = (mtlA[i * 4] * mtlB[2]) + (mtlA[i * 4 + 1] * mtlB[6])
					+ (mtlA[i * 4 + 2] * mtlB[2 * 4 + 2]) + (mtlA[i * 4 + 3] * mtlB[3 * 4 + 2]);

			tmp[i * MATRIX_SIDE + 3] = (mtlA[i * 4] * mtlB[3]) + (mtlA[i * 4 + 1] * mtlB[7])
					+ (mtlA[i * 4 + 2] * mtlB[2 * 4 + 3]) + (mtlA[i * 4 + 3] * mtlB[3 * 4 + 3]);
		}
	...
	/**
	 * The in the transformation the matrix that allow to get the
	 * transformations of the world
	 *
	 * @param modelView
	 *            Transformation over the model
	 * @param perspective
	 *            Transformation over the perspective
	 */
	 ...
	 *
	 * @param x
	 *            scaling vector in the x-axle
	 * @param y
	 *            scaling vector in the y-axle
	 * @param z
	 *            scaling vector in the z-axle
	 */
	 ...
	 *
	 * @param x
	 *            translation vector in the x-axle
	 * @param y
	 *            translation vector in the y-axle
	 * @param z
	 *            translation vector in the z-axle
	 */
